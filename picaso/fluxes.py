...
@jit(nopython=True, cache=True)
def get_kzz(pressure, temp,grav,mmw,tidal,flux_net_ir_layer, flux_plus_ir_attop,t_table, p_table, grad, cp, calc_type,nstr, opacityclass, moist = None):
...
#     convergence.
    ...
    for j in range(len(pressure)-1):
        ...
        if moist == True:
            grad_x,cp_x = moist_grad(tbar, pbar, t_table, p_table, grad, cp, calc_type, opacityclass)
        else:
