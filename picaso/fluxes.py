	...
	for ng in range(numg):
		for nt in range(numt):
			...
			if stream==2:
				...
				M, B, A_int, N_int, F, G = setup_2_stream_banded(nlayer, wno, nwno, w0, b_top, b_surface, 
				surf_reflect, F0PI, u0, dtau, tau, w_single, w_multi, u1, P) 
				A, N = setup_2_stream_banded(nlayer, wno, nwno, w0, b_top, b_surface, 
				surf_reflect, F0PI, u0, dtau, tau, w_single, w_multi, u1, P, calculate=2) 
				F1, G1 = setup_2_stream_banded(nlayer, wno, nwno, w0, b_top, b_surface, 
				surf_reflect, F0PI, u0, dtau, tau, w_single, w_multi, u1, P, calculate=1) 
			else:
				...
				M, B, A_int, N_int, F, G = setup_4_stream_banded(nlayer, wno, nwno, w0, b_top, b_surface, surf_reflect, F0PI, 
						u0, dtau, tau, w_single, w_multi, u1, P)
			...
			for i in range(nlayer):
				for l in range(stream):
					sing_scat[i,:] = sing_scat[i,:] + w_single[l][i,:] * P(u1)[l] * intgrl_new[stream*i+l,:]
...
def get_thermal_new(nlevel, wno, nwno, numg, numt, tlevel, dtau, tau, w0, cosb, 
	...
	for ng in range(numg):
		for nt in range(numt):
			if stream == 2:
				M, B, A_int, N_int, F, G, Q, lamda = setup_2_stream_banded(nlayer, wno, nwno, w0, b_top, b_surface, 
				surf_reflect, 0, ubar1[ng, nt], dtau, tau, w_single, w_multi, ubar1[ng,nt], P, b0, b1, calculation='thermal')
			else:
				M, B, A_int, N_int, F, G = setup_4_stream_banded(nlayer, wno, nwno, w0, b_top, b_surface, 
				surf_reflect, 0, ubar1[ng, nt], dtau, tau, w_single, w_multi, ubar1[ng,nt], P, b0, b1, calculation='thermal')
			...
			#========================= Start loop over wavelength =========================
			for W in range(nwno):
				(intgrl_new[:,W], flux_bot[W], X[:,W]) = solve_4_stream_banded(M[:,:,W], B[:,W],  
				A_int[:,:,W], N_int[:,W], F[:,W], G[W], stream)
			...
			if stream == 2:
				sing_scat1 = ubar1[ng,nt]*((w_single[0] - w_single[1]*ubar1[ng,nt]*Q) * X[::2,:] / (1+lamda*ubar1[ng,nt])
						* (1-exp(-dtau_og * (1/ubar1[ng,nt] + lamda)))
						+ (w_single[0] + w_single[1]*ubar1[ng,nt]*Q) * X[1::2,:] / (1-lamda*ubar1[ng,nt])
						* (1-exp(-dtau_og * (1/ubar1[ng,nt] - lamda)))
						+ pi*(1-w0)/(1-w_single[0]*w0)*((w_single[0]*b0+(cosb/(1-w0*cosb))*b1*ubar1[ng,nt])*(1-expdtau)
						+ w_single[0]*b1*(ubar1[ng,nt] - (dtau_og + ubar1[ng,nt]) * expdtau)))

				#intgrl_per_layer1 = ubar1[ng,nt]*(w0*(w_single[0] - w_single[1]*ubar1[ng,nt]*Q) * X[::2,:] / (1+lamda*ubar1[ng,nt])
				#		* (1-exp(-dtau_og * (1/ubar1[ng,nt] + lamda)))
				#		+ w0*(w_single[0] + w_single[1]*ubar1[ng,nt]*Q) * X[1::2,:] / (1-lamda*ubar1[ng,nt])
				#		* (1-exp(-dtau_og * (1/ubar1[ng,nt] - lamda)))
				#		+ (b0 + cosb_og/(1-w0*cosb_og) * b1) * (1 - expdtau)
				#		+ b1 * (ubar1[ng,nt] - (dtau_og + ubar1[ng,nt]) * expdtau))
			intgrl_per_layer = (w0 *  sing_scat1 #* pi 
...
def setup_2_stream_banded(nlayer, wno, nwno, w0, b_top, b_surface, surf_reflect, F0PI, ubar0, dtau,tau, 
				w_single, w_multi, ubar1, P, B0=0., B1=0., calculate=0,calculation='reflected'):
	if P is None:
		def P(mu): # Legendre polynomials
			return [1, mu, 1/2 * (3*mu**2 - 1), 1/2 * (5*mu**3 - 3*mu), 1/8 * (35*mu**4 - 30*mu**2+3)]
	...
	if calculation is "reflected":
		...
		zmn_down = 2*pi * (1-w0)/a[0] * (B0/2 - B1/a[1]  )
		zmn_up = 2*pi * (1-w0)/a[0] * (B0/2 - B1/a[1] + B1*dtau/2)
		zpl_down = 2*pi * (1-w0)/a[0] * (B0/2 + B1/a[1]  )
		zpl_up = 2*pi * (1-w0)/a[0] * (B0/2 + B1/a[1] + B1*dtau/2)
	...
	if calculate == 0: # full picaso atmosphere calculation
		#	construct matrices
		...
		Mb[1,nn[2::2],:] = -Q1[1:,:]
		Mb[1, nn[3::2],:] = -Q1[1:,:]
		Mb[2,nn[1:-1:2],:] = Q2pl[:-1,:]
		Mb[2, nn[2::2],:] = -Q2[1:,:]
		Mb[3,nn[:-2:2],:] = Q1mn[:-1,:]
		Mb[3,nn[1:-1:2],:] = Q1pl[:-1,:]
		Mb[4,nn[:-2:2],:] = Q2mn[:-1,:]
		B[nn[1:-1:2],:] = zmn_down[1:,:] - zmn_up[:-1,:]
		B[nn[2::2],:] = zpl_down[1:,:] - zpl_up[:-1,:]
		...
		if calculation is 'reflected':
			...
			N_int[nn[::2],:] = (1-w0) * ubar1 / a[0] * ( B0*(1-expdtau) + B1*(ubar1 - (dtau+ubar1)*expdtau))
			N_int[nn[1::2],:] = (1-w0) * ubar1 / a[0] * ( B1*(1-expdtau) / a[1])
		...
		return Mb, B, A_int, N_int, F_bot, G_bot#, Q1, Q2
...
def setup_4_stream_banded(nlayer, wno, nwno, w0, b_top, b_surface, surf_reflect, F0PI, ubar0, dtau,tau, 
	...
	if calculation is 'reflected':
		...
		z1mn_up = 2*pi * (1-w0)/a[0] * (B0/2 - B1/a[1] + B1*dtau/2) 
		z2mn_up = -pi * (1-w0) / (4*a[0]) * (B0 + B1*dtau) 
		z1pl_up = 2*pi * (1-w0)/a[0] * (B0/2 + B1/a[1] + B1*dtau/2) 
		z2pl_up = -pi * (1-w0) / (4*a[0]) * (B0 + B1*dtau) 
