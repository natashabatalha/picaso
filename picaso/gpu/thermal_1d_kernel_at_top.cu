...
__global__ void setup_tri_diag_last(int nlayer,int nwno,  double *c_plus_up_dev, double *c_minus_up_dev,double *c_plus_down_dev, double *c_minus_down_dev,
   ...
   if ( (index > (nlayer-1)*nwno) && (index < nlayer*nwno) ){
     ...
     A_odd_dev[index+nlayer*nwno] = e1_dev[index]-atm_surf_reflect*e3_dev[index];
     B_odd_dev[index+nlayer*nwno] = e2_dev[index]-atm_surf_reflect*e4_dev[index];
     C_odd_dev[index+nlayer*nwno] = 0.0;
     D_odd_dev[index+nlayer*nwno] = b_surface_dev[index -(nlayer-1)*nwno ]-c_plus_down_dev[index] + atm_surf_reflect*c_minus_down_dev[index];
...
__global__ void update_flux_combined_one_kernel_loop_new(
    const double * __restrict__ flux_plus_dev,     // bottom boundary only
    double * __restrict__ flux_plus_mdpt_dev,      // ONLY stores top boundary
    ...
    int nwno)
{
    ...
    // ---------------------------------------------------------------------
    // Load bottom boundary flux_plus
    // ---------------------------------------------------------------------
    int idx_bottom = (nlevel - 1) * nwno + wn;
    double f_plus = flux_plus_dev[idx_bottom];   // ONLY READ

    double f_plus_mdpt = 0.0;    // will store the top midpoint flux after the loop

    // Sweep upward: ibot = nlevel - 2 ... 0
    for (int itop = 0; itop < nlevel - 1; ++itop) {

        int ibot = nlevel - 2 - itop;
        int idx_ibot = ibot * nwno + wn;

        // --- cache per-layer values ---
        double lam     = lamda_dev[idx_ibot];
        double dtau    = dtau_og_dev[idx_ibot];
        double lamubar = lam * ubar1;

        double ea      = exptrm_angle_dev[idx_ibot];
        double ea_m    = exptrm_angle_mdpt_dev[idx_ibot];

        double ep      = exptrm_positive_dev[idx_ibot];
        double ep_m    = exptrm_positive_mdpt_dev[idx_ibot];
        double em      = exptrm_minus_dev[idx_ibot];
        double em_m    = exptrm_minus_mdpt_dev[idx_ibot];

        double G       = G_matrix_dev[idx_ibot];
        double H       = H_matrix_dev[idx_ibot];
        double a1      = alpha1_matrix_dev[idx_ibot];
        double a2      = alpha2_matrix_dev[idx_ibot];

        double denomGp = 1.0 / (lamubar - 1.0);
        double denomHp = 1.0 / (lamubar + 1.0);

        // ---- Step 3: F_plus update ----
        double termG = G * denomGp * (ep * ea - 1.0);
        double termH = H * denomHp * (1.0 - em * ea);
        double termS = a1 * (1.0 - ea)
                     + a2 * (ubar1 - (dtau + ubar1) * ea);

        double f_plus_new = f_plus * ea + termG + termH + termS;

        // ---- Step 4: midpoint update ----
        double termG_m = G * denomGp * (ep * ea_m - ep_m);
        double termH_m = - H * denomHp * (em * ea_m - em_m);
        double termS_m = a1 * (1.0 - ea_m)
                       + a2 * (ubar1 + 0.5 * dtau - (dtau + ubar1) * ea_m);

        f_plus_mdpt = f_plus * ea_m + termG_m + termH_m + termS_m;
    ...
    // ---------------------------------------------------------------------
    // Only save TOP midpoint flux
    // ---------------------------------------------------------------------
    flux_plus_mdpt_dev[wn] = f_plus_mdpt;
}
...
__global__ void get_fluxes_at_top(
    double *flux_at_top_dev,
    const double *flux_plus_mdpt_dev,
    int i_iter,
    int nwno)
{
    int wn = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    flux_at_top_dev[i_iter * nwno + wn] = flux_plus_mdpt_dev[wn];
}
    ...
    int    nt)
{
    ...
    size_t size_flux_lvl_top = (size_t)nwno * sizeof(double);
    ...
    CUDA_CHECK(cudaMalloc(&ctx.flux_plus_mdpt_dev,  size_flux_lvl_top));
    ...
    )
{
    ...
    setup_tri_diag_all<<<grid_layer, blockSize>>>(
        ...
        ctx.c_plus_up_dev, ctx.c_minus_up_dev,
        ctx.c_plus_down_dev, ctx.c_minus_down_dev,
        ctx.b_top_dev, ctx.b_surface_dev, surf_reflect,
    ...
    for (int i_iter = 0; i_iter < ng * nt; ++i_iter) {
        ...
        int gridSize = (nwno + blockSize - 1) / blockSize;

        get_fluxes_at_top<<<gridSize, blockSize>>>(
            ctx.flux_at_top_dev,
