    ...
    if nstr[3] != 0:
        ...
        for i in range(nstr[3], nstr[4]):
            ...
            if start_index == end_index:
                kz_lower.append(np.mean(kz[start_index:end_index+1])) # this is a one layer convective zone
            else: 
...
@jit(nopython=True, cache=True)
def get_f_vec(nstr, nofczns, temp, flux_net, flux_net_midpt):
    """
    Maps net fluxes at radiative levels and layers to the residual vector f_vec.
    ...
    temp : array
        Temperature profile.
    flux_net : array
        Net fluxes at levels.
    flux_net_midpt : array
        Net fluxes at layer midpoints.
    ...
    f_vec : array
        Residual vector.
    n_total : int
        Total number of radiative points.
    sum_f2 : float
        Sum of squared residuals.
    sum_temp2 : float
        Sum of squared temperatures at radiative points.
    test : float
    ...
    nao = n_top_r
    n_total = 0
    f_vec = np.zeros(len(flux_net)) # initialize with max possible size
    ...
    for nca in range(0, 3 * nofczns, 3):
        ...
        for j in range(n_top_a + 1, n_strt_a + 1):
            ...
            sum_f2 += f_vec[j - nao]**2
            sum_temp2 += temp[j - nao]**2 # preserved original logic
            if abs(f_vec[j - nao]) > test:
                test = abs(f_vec[j - nao])
            n_total += 1
    ...
    return f_vec[:n_total], n_total, sum_f2, sum_temp2, test

@jit(nopython=True, cache=True)
def compute_jacobian(temp_old, nstr, nofczns, n_total, rfaci, rfacv, tidal,
                     ...
                     moist, eps, flux_net_old, flux_net_midpt_old):
    """
    Computes the Jacobian matrix for the radiative layers.
    ...
    n_total : int
        Total number of radiative points.
    rfaci, rfacv : float
        IR and Visible flux addition fractions.
    tidal : array
        Tidal flux component.
    Atmosphere, OpacityWEd, OpacityNoEd, ScatteringPhase, Disco, Opagrid, AdiabatBundle : namedtuples
        Atmospheric and radiative information.
    F0PI : array
        Stellar spectrum.
    compute_reflected, compute_thermal : bool
        Flags for radiative transfer components.
    do_holes : bool
        Flag for patchy clouds.
    fhole : float
        Hole fraction.
    hole_OpacityWEd, hole_OpacityNoEd : namedtuples
        Clear sky opacities.
    moist : bool
        Flag for moist adiabat.
    eps : float
        Perturbation factor.
    flux_net_old, flux_net_midpt_old : array
        Current net fluxes.
    ...
    for nz in range(0, 3 * nofczns, 3):
        ...
        for jm in range(n_top, n_strt + 1):
            ...
            if do_holes == True:
                flux_results = get_fluxes(Atmosphere_perturbed, OpacityWEd, OpacityNoEd, ScatteringPhase,
                            Disco, Opagrid, F0PI, False, compute_thermal, # False for reflected as in original
...
def get_residuals_at_temp(temp, nstr, nofczns, rfaci, rfacv, tidal,
    ...
    flux_net = rfaci * flux_net_ir + rfacv * flux_net_v + tidal
    flux_net_midpt = rfaci * flux_net_ir_layer + rfacv * flux_net_v_layer + tidal

    f_vec, n_total, sum_f2, sum_temp2, test = get_f_vec(nstr, nofczns, temp, flux_net, flux_net_midpt)
    return f_vec, n_total, sum_f2, sum_temp2, test, flux_results

@jit(nopython=True, cache=True)
def newton_raphson_solver(temp, nstr, nofczns, convergence_criteria,
                          rfaci, rfacv, tidal,
                          Atmosphere, OpacityWEd, OpacityNoEd, ScatteringPhase, Disco, Opagrid, AdiabatBundle,
    ...
    tmin, tmax = Opagrid.tmin, Opagrid.tmax
    ...
    for its in range(it_max):
        ...
        # Store old state
        temp_old = temp.copy()
        flux_net_ir_old = flux_results[5].copy()
        ...
        # Compute gradient and Newton direction
        g = np.zeros(n_total)
        p = np.zeros(n_total)
        ...
        while flag_converge == 0:
            ...
            for j1 in range(n_top_r + 1, nlevel):
                if temp[j1] < tmin: temp[j1] = tmin + 0.1
            ...
            if alam < alamin:
                check = True
                flag_converge, check = check_convergence(f_vec, n_total, tolf, check, f, -p, tolmin, temp, temp_old, g, tolx)
                ...
                if alam == 1.0:
                    ...
                    if anr == 0:
                        ...
                        elif b <= 0.0: tmplam = (-b + sqrt(disc)) / (3.0 * anr)
                        else: tmplam = -slope / (b + sqrt(disc))
                    if tmplam > 0.5 * alam: tmplam = 0.5 * alam

            if flag_converge != 2 and flag_converge != 1:
                alam2 = alam
                f2 = f
...
def t_start(nofczns,nstr,convergence_criteria,# 
    ...
    tmin =Opagrid.tmin 
    tmax = Opagrid.tmax 
    max_inner_iterations,inner_threshold,step_multiplier = convergence_criteria.it_max,convergence_criteria.conv,convergence_criteria.x_max_mult#convergence_criteria.max_inner_iterations,convergence_criteria.inner_threshold,convergence_criteria.step_multiplier
...
@jit(nopython=True, cache=True)
def t_start_WIP(nofczns, nstr, convergence_criteria,
            rfaci, rfacv, tidal,
            Atmosphere, OpacityWEd, OpacityNoEd, ScatteringPhase, Disco, Opagrid, AdiabatBundle,
            F0PI,
            save_profile, all_profiles,
            fhole=None, hole_OpacityWEd=None, hole_OpacityNoEd=None,
            verbose=1, do_holes=None,
            moist=False, egp_stepmax=False, solver='newton_raphson'):
    """
    Module to iterate on the level TP profile to make the Net Flux as close to 0.
    Opacities/chemistry are not updated while iterating in this module.
    ...
    rfacv : float
        Visible flux addition fraction.
    tidal : ndarray
        effectively sigmaTeff^4.
    Atmosphere : namedtuple
        PT profile, chemistry, etc.
    OpacityWEd, OpacityNoEd, ScatteringPhase, Disco, Opagrid, AdiabatBundle : namedtuples
        Radiative and atmospheric data.
    F0PI : ndarray
        Stellar spectrum.
    save_profile : bool
        If True, save all intermediate profiles.
    all_profiles : ndarray
        All saved profiles.
    fhole, hole_OpacityWEd, hole_OpacityNoEd : optional
        Hole fraction and clear sky opacities.
    verbose : int
        Verbosity level.
    do_holes : bool
        If True, use patchy cloud model.
    moist : bool
        If True, use moist adiabat.
    egp_stepmax : bool
        If True, use unbounded stepmax.
    solver : str
        The non-linear solver to use (default: 'newton_raphson').
...
def get_fluxes(Atmosphere, OpacityWEd, OpacityNoEd,ScatteringPhase,
    ...
    surf_reflect, hard_surface, single_phase,multi_phase,frac_a,frac_b,frac_c,constant_back,constant_forward = ScatteringPhase.surf_reflect, ScatteringPhase.hard_surface, ScatteringPhase.single_phase,ScatteringPhase.multi_phase,ScatteringPhase.frac_a,ScatteringPhase.frac_b,ScatteringPhase.frac_c,ScatteringPhase.constant_back,ScatteringPhase.constant_forward
    #unpack disco items 
...
def update_rfacv(bundle, Atmosphere, OpacityWEd, OpacityNoEd=None,
                 ...
                 verbose=True):
    """
    Update rfacv following Koll2022 ApJ 924 134 by using the get_fluxes routine. 
    Intended for use with tidally locked rocky exoplanets.
    ...
    OpacityNoEd_clear : namedtuple
        The clear opacities / scattering properties w/o delta eddington correction (opposed to the cloudy ones which are stored in the main opacity tuple)
    verbose : bool
        Flag for printing message to update rfacv
    """

    # print update message if requested
    if verbose: print("I am updating rfacv.")

    # unpack items needed to update rfacv
    # properties or star and orbit
    ...
    # opacity items w/ delta eddington correction
    ...
    # calculate the bond albedo using net visible fluxes and the solar constant egp estimate
    solarc = c.sigma_sb.to(u.erg/u.K**4/u.cm**2/u.s).value * temp_star**4 * (radius_star/sa)**2 # solar constant S=L/(4 pi a^2)=sigma_SB * T_star^4 * (R_star / a)^2
    bond_albedo = 1 + 2*FNETV/solarc
...
ScatteringPhase_Tuple = namedtuple('ScatteringPhase_Tuple',['surf_reflect', 'hard_surface', 'single_phase','multi_phase','frac_a','frac_b','frac_c','constant_back','constant_forward'])
...
def calculate_atm(bundle, opacityclass, only_atmosphere=False):
    ...
    ScatteringPhase= ScatteringPhase_Tuple(atm.surf_reflect,atm.hard_surface,single_phase,multi_phase,frac_a,frac_b,frac_c,constant_back,constant_forward)
...
def profile(bundle, nofczns, nstr, temp, pressure, 
    ...
    do_quench_appox = bundle.inputs['approx']['chem_params']['quench']
   ...
   #under what circumstances to do we compute a self consistent kzz calc 
    ...
    if do_kzz_calc:
        ...
        bundle.inputs['atmosphere']['kzz']['sc_kzz']=kz #bookkeeping current kz 
    ...
    ### 3) IF: ANALYTIC HEAT REDISTRIBUTION
    if analytic_rfacv:
        rfacv = update_rfacv(bundle=bundle, Atmosphere=Atmosphere, OpacityWEd=OpacityWEd, OpacityNoEd=OpacityNoEd,
                             ScatteringPhase=ScatteringPhase, Disco=Disco, Opagrid=Opagrid, F0PI=F0PI,
                             do_holes=do_holes, fhole=fhole, OpacityWEd_clear=OpacityWEd_clear, OpacityNoEd_clear=OpacityNoEd_clear)
        bundle.inputs['climate']['rfacv'] = rfacv #bookkeeping current rfacv
