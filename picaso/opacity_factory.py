...
def restruct_continuum(original_file,colnames, new_wno, overwrite=False,new_db=os.path.join(__refdata__, 'opacities','continuum.db')):
    """
...
def get_original_data(original_file,colnames, overwrite=False,new_db=os.path.join(__refdata__, 'opacities','continuum.db')):
    """
    The continuum factory takes the CIA opacity file and adds in extra sources of 
    opacity from other references to fill in empty bands. It assumes that the original file is 
    structured as following,with the first column wavenumber and the rest molecules: 
...
class ContinuumFactory_OLD_AF():
    ...
    """
    def __init__(self, original_file,colnames, new_wno, overwrite=False,new_db=os.path.join(__refdata__, 'opacities','continuum.db')):
        ...
        if os.path.exists(new_db):
            if overwrite:
                raise Exception("Overwrite is set to false to save db's from being overwritten.")
        self.db_cia = {i:{} for i in self.molecules}
    ...
    def insert(self,cur,conn,mol,T,opacity):
        """Insert into """
        cur.execute('INSERT INTO continuum (molecule, temperature, opacity) values (?,?,?)', (mol,float(T), opacity))

    def restructure_opacity(self):
        #start by opening connection 
        cur, conn = open_local(self.new_db)
        ...
        for i in range(self.ntemp): 
            for m in self.molecules:
                ...
                if m=='H2H2':
                    #first add h2h2 overtone band 
                    h2h2, loc, add = self.h2h2_overtone(self.temperatures[i],self.new_wno)
                    if add:
                        new_bundle[loc] = h2h2
                ...
                self.insert(cur,conn,m, self.temperatures[i], new_bundle)
        ...
        for i in range(self.ntemp): 
            ...
            self.insert(cur,conn,'H2-', self.temperatures[i], bundle)

        #get hminusbf 
        #self.db_cia['H-bf'] = {}
        for i in range(self.ntemp):
            if self.temperatures[i]<600.0:
                bundle = np.zeros(len(self.new_wno)) + 1e-33
            else:
                bundle = self.get_hminusbf(self.new_wno)
            self.insert(cur,conn,'H-bf', self.temperatures[i], bundle)

        #get hminusff 
        for i in range(self.ntemp):
            bundle = self.get_hminusff(self.temperatures[i], self.new_wno) 
            self.insert(cur,conn,'H-ff', self.temperatures[i], bundle)


        conn.commit()
        conn.close()

    def restructure_opacity_faster(self):
        #start by opening connection 
        cur, conn = open_local(self.new_db)
    ...
    def get_h2minus(self, t, wno):
        ...
        h2m_interpolator =  RegularGridInterpolator((df.index.values,wno_bell), df.values, bounds_error=False,fill_value=None)#)bounds_error=False,fill_value=None)
