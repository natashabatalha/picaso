...
def get_data_config():
    ...
    config_file = os.path.join(__refdata__, 'config.json') if __refdata__ else ''
    ...
    opacities_files = inputs.get('opacities', {}).get('files', {})
    resampled_default = opacities_files.get('resampled-default', 'opacities')
    resampled = opacities_files.get('resampled', 'opacities/resampled')
    ktable_by_molecule = opacities_files.get('ktable_by_molecule', 'opacities/resortrebin')
    preweighted = opacities_files.get('preweighted', 'opacities/preweighted')

    resampled_dest = os.path.join(__refdata__, resampled) if not os.path.isabs(resampled) else resampled
    ...
    "resampled_opacity":{
        'default':{
           'url':{'opacities_0.3_15_R15000.db.tar.gz':'https://zenodo.org/records/14861730/files/opacities_0.3_15_R15000.db.tar.gz'},
            ...
            'default_destination':os.path.join(__refdata__, resampled_default) 
        },
        'R60000,0.6-6um':{
            ...
            'default_destination':resampled_dest
            },
        'R20000,4.8-15um':{
    ...
    'ck_tables':{
        'by-molecule':{
            ...
            'default_destination':os.path.join(__refdata__, ktable_by_molecule) ,
        },
            
        'pre-weighted':{
            'url':{"sonora_2121grid.zip": "https://zenodo.org/records/18636725/files/sonora_2121grid.zip",
                   "sonora_2121grid_NoTiOVO.zip": "https://zenodo.org/records/18636725/files/sonora_2121grid_NoTiOVO.zip"},
            'description':'196 CK Tables in new H5 file structure.',
            'default_destination':os.path.join(__refdata__, preweighted) ,
...
def get_reference(path_to_picaso_refdata):
    ...
    __refdata__ = os.environ.get('picaso_refdata')
    if __refdata__:
        same_path = os.path.abspath(os.path.normpath(path_to_picaso_refdata)) == os.path.abspath(os.path.normpath(__refdata__))
        if same_path and os.path.exists(os.path.join(__refdata__, 'config.json')):
            raise Exception('It looks like you already have the default reference directory. This will overwrite all the files in your reference folder. If you would like to proceed, clear your reference folder and then rerun this function again.')
