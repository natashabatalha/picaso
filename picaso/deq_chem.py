...
# this is the CO- CH4 - H2O quench level
    t_chem_co = (3.0e-6/pressure)*np.exp(42000/temp) ## level chemical timescale
...
# start from the CO- CH4 - H2O quench level and search

    t_chem_co2 = (1e-10/(pressure**0.5))*np.exp(38000./temp)
...
# now calculate the NH3/N2 quench level
    ...
    t_chem_nh3 = (1e-7/pressure)*np.exp(52000/temp)
...
# now calculate the HCN quench level
    ...
    t_chem_hcn = (1.5e-4/(pressure*(3.**0.7)))*np.exp(36000./temp)
...
def mix_all_gases(kappa1,kappa2,kappa3,kappa4,kappa5,mix1,mix2,mix3,mix4,mix5,gauss_pts, gauss_wts,indices):
    ...
    Nk=len(gauss_wts)
    Nlayer = len(indices[0])
    
    # :,:,0:self.nwno,0:self.ngauss
    kappa_mixed = np.zeros(shape=(Nlayer,kappa1.shape[2],Nk,4)) 

    for ilayer in range(Nlayer):
        ct =0
        for p_ind in [indices[0][ilayer],indices[1][ilayer]]:
            for t_ind in [indices[2][ilayer],indices[3][ilayer]]: 
                for iw in range(kappa1.shape[2]):
...
def do_mixing_mono(kappa1_mono,kappa2_mono,kappa3_mono,kappa4_mono,kappa5_mono,
    ...
    kmix_bin,mix_t =mix_2_gases(np.exp(kappa1_mono),np.exp(kappa2_mono), mix1,mix2,gauss_pts,gauss_wts)
    
    kmix_bin,mix_t = mix_2_gases(kmix_bin,np.exp(kappa3_mono), mix_t,mix3,gauss_pts,gauss_wts)
    
    kmix_bin,mix_t = mix_2_gases(kmix_bin,np.exp(kappa4_mono), mix_t,mix4,gauss_pts,gauss_wts)
    
    kmix_bin,mix_t = mix_2_gases(kmix_bin,np.exp(kappa5_mono), mix_t,mix5,gauss_pts,gauss_wts)
    '''
