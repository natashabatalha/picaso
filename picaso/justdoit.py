    ...
    if dimension == '1d':
        ...
        if  'reflected' in calculation:
            if __hardware__ == 'gpu':
                ...
                DTAU_gpu = cp.asarray(DTAU).reshape(-1)
                TAU_gpu = cp.asarray(TAU).reshape(-1)
                W0_gpu = cp.asarray(W0).reshape(-1)
                COSB_gpu = cp.asarray(COSB).reshape(-1)
                GCOS2_gpu = cp.asarray(GCOS2).reshape(-1)
                ftau_cld_gpu = cp.asarray(ftau_cld).reshape(-1)
                ftau_ray_gpu = cp.asarray(ftau_ray).reshape(-1)
                DTAU_OG_gpu = cp.asarray(DTAU_OG).reshape(-1)
                TAU_OG_gpu = cp.asarray(TAU_OG).reshape(-1)
                W0_OG_gpu = cp.asarray(W0_OG).reshape(-1)
                COSB_OG_gpu = cp.asarray(COSB_OG).reshape(-1)
                ...
                if do_holes:
                    DTAU_clear_gpu = cp.asarray(DTAU_clear).reshape(-1)
                    TAU_clear_gpu = cp.asarray(TAU_clear).reshape(-1)
                    W0_clear_gpu = cp.asarray(W0_clear).reshape(-1)
                    COSB_clear_gpu = cp.asarray(COSB_clear).reshape(-1)
                    DTAU_OG_clear_gpu = cp.asarray(DTAU_OG_clear).reshape(-1)
                    TAU_OG_clear_gpu = cp.asarray(TAU_OG_clear).reshape(-1)
                    W0_OG_clear_gpu = cp.asarray(W0_OG_clear).reshape(-1)
                    COSB_OG_clear_gpu = cp.asarray(COSB_OG_clear).reshape(-1)
            xint_at_top = 0 
