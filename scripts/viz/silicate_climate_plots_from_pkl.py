...
# %%
fnames = os.listdir("../data/four_clouds_testing/pkl_outputs")
...
for fname in tqdm(fnames):
    with open(f"../data/four_clouds_testing/pkl_outputs/{fname}", 'rb') as file:
        out = pickle.load(file)
        teff_match = re.search(r'teff_(\d+)', fname)
        if teff_match:
            teff = int(teff_match.group(1))
        fsed_match = re.search(r'fsed_(\d+)', fname)
        if fsed_match:
            fsed = int(fsed_match.group(1))
        pressure_bobcat, temp_bobcat = np.loadtxt(jdi.os.path.join(sonora_profile_db, f"t{teff}g{grav}nc_m0.0.dat"),
                    usecols=[1, 2], unpack=True, skiprows=1)
        cl_run.inputs_climate(temp_guess= temp_bobcat, pressure= pressure_bobcat, nstr = nstr, nofczns = nofczns , rfacv = rfacv, cloudy = "cloudless", mh = '0.0', CtoO = '1.0',species = ['MgSiO3'], fsed = fsed, beta = 0.1, virga_param = 'const', mieff_dir = "~/projects/clouds/virga/refrind", do_holes = False, fhole = 0.5, fthin_cld = 0.9, moistgrad = False)
        climate_plots(out, cl_run, "../data/four_clouds_testing/pkl_output_plots/", "../data/four_clouds_testing/pkl_output_plots/", fname, temp_bobcat,pressure_bobcat,teff,
