...
@bind nstr_upper Select([55, 68, 77, 88])

# ╔═╡ e168698a-56e4-45c6-8b38-af9a622c0adf
@bind teff Select([1100, 1600, 2200])

# ╔═╡ 0b540bae-12f7-4d52-9a2a-d3c4686742ee
@bind fsed Select([1, 2, 3, 4])

# ╔═╡ ad69062c-5cea-48bd-a6ce-1ae6cab8f9ad
all_files = readdir("../data/convh5");

# ╔═╡ 928e3931-4358-4131-b945-ca4b98218ff6
fname = last(filter(x -> startswith(x, "convergence_fsed$(fsed)_teff$(teff)_nstrupper$(nstr_upper)"), all_files));

# ╔═╡ ef9f4c8f-0896-46d9-99b8-4cc84dc3183d
begin
	f = h5open("../data/convh5/$fname", "r")
...
function plot_temp_cond(i)
	...
	plot!(twiny(), total_condmmr[:,i] .+ 1e-10, pressure[1:90], xscale=:log10, yscale=:log10, yflip=true, xlabel="Total condensate MMR", xlim=(1e-10, 1), title="fsed=$fsed, teff=$teff, nstr_upper=$nstr_upper, iteration $i", legend=nothing, color=:teal, xguidefont=font(:teal))
