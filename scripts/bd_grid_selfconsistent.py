...
for nstr_upper in [55, 68, 77, 88]:
    for teff in [1100, 1600, 2200]:
        ...
        try:
            ...
            with h5py.File(f"data/convh5_oktemp/convergence_fsed{fsed}_teff{teff}_nstrupper{nstr_upper}_cloudmode{cloudmode}_dt{tstamp}.h5", "w") as f:
                p_picaso = f.create_dataset("pressure_picaso", data=out_selfconsistent["pressure"])
                p_virga = f.create_dataset("pressure_virga", data=out_selfconsistent["cld"][-1]["pressure"])
                f.create_dataset("nstrs", data=np.array(out_selfconsistent["nstr"]))
                for k in cloud_outputs:
                    f.create_dataset(k, data=cloud_outputs[k])
                f.create_dataset("altitude_virga", data=out_selfconsistent["cld"][-1]["altitude"].shape)
                ...
                t = f.create_dataset("temperature_picaso", data=out_selfconsistent["all_profiles"])
        except ValueError:
            with open(f"data/convh5_oktemp/fsed{fsed}_teff{teff}_nstrupper{nstr_upper}.txt", "w") as f:
