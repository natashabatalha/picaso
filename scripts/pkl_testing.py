...
for subfolder in ["selfconsistent"]:
    ...
    for f in ["profile", "get_fluxes", "find_strat"]:
        ...
        for (n, rnew, rsaved) in zip(names, res_new, result):
            if isinstance(rnew, dict):
                matches = matches and np.all([equal_check(rnew[k], rsaved[k]) for k in rsaved.keys()])
            else:
                matches = matches and equal_check(rnew, rsaved)
