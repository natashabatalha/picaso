...
C      implicit double precision (a-h,o-z)
      implicit none
      EXTERNAL LOCATE, DIDGRAD, DIDCP
      INTEGER N, I, J
      REAL*8 P_TABLE(26), T_TABLE(53), T_SEARCH, P_SEARCH
      REAL*8 GRAD(53,26), CP(53,26), GRADX, Ttest, Ptest, CPX
  ...
  4   CONTINUE
      ...
      CALL didcp(Ttest, Ptest,CPX, T_table, P_TABLE, CP)
...
10	IF (JU-JL.GT.1) THEN
	...
	END
      ...
      subroutine didcp(t,p,cpx,tlog,plog,cp)
         implicit double precision (a-h,o-z)
         REAL*8 tlog(53),plog(26),cp(53,26)
...
c     Now interpolate in p & t
        cpx = (1.d0-factkt)*(1.d0-factkp)*cp1 +
     1   factkt*(1.d0-factkp)*cp2 + factkt*factkp*cp3 + 
     2   (1.d0-factkt)*factkp*cp4
        cpx = 10.d0**cpx
      RETURN
      END
